...
ifeq ($(RUST_BACKEND),ironox)
	...
	CLIF_SO := librustc_codegen_cranelift.dylib
	BACKEND := $(realpath $(CLIF_DIR)/target/release/$(CLIF_SO))
	CLIF_FLAGS :=  --sysroot $(realpath $(CLIF_DIR)/build_sysroot/sysroot) -Zcodegen-backend=$(BACKEND)
	CARGO_FLAGS := +nightly
	CARGO := cargo
	...
	BACKEND := $((rustc +nightly --print sysroot))/x86_64-unknown-linux-gnu/stage1/lib/rustlib/x86_64-unknown-linux-gnu/codegen-backends/$(LLVM_SO)
	CUSTOM_RUSTC := RUSTC=rustc
...
NIGHTLY_FLAGS := -Zno-landing-pads
...
build:
	LD_LIBRARY_PATH=${LIBS} RUSTFLAGS="${RUSTFLAGS}" ${CUSTOM_RUSTC} ${CARGO} ${CARGO_FLAGS} build ${MANIFEST_PATH} 2>&1
